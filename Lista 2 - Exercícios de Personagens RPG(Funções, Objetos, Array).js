...
function buscaPersonagemDoTipo(tipo) {
    let tipos = tiposDePersonagem();
    tipo = tipo.toUpperCase();

    let filtroPersonagens =
        tipos.indexOf(tipo) >= 0
            ? personagens.filter((personagem) => personagem.type == tipo)
            : `Tipo inválido. Tipos existentes para busca: ${tipos}`;

    return `Personagens do tipo ${tipo}: ${filtroPersonagens.map(
        (personagem) => personagem.full_name
    )}`;
}

console.log(buscaPersonagemDoTipo("MYSTIQUE"));
...
function tiposDeRaridadeDePoderes() {
    let tipos = [];
    personagens.forEach((personagem) => {
        personagem.powers.forEach((poder) =>
            tipos.indexOf(poder.rarity) == -1 ? tipos.push(poder.rarity) : ""
        );
    });
    return tipos;
}
console.log(tiposDeRaridadeDePoderes());

function buscaPersonagensComTipoDePoder(tipoDePoder) {
    tipoDePoder = tipoDePoder.toUpperCase();
    let tiposDePoderes = tiposDeRaridadeDePoderes();
    let nomeDosPersonagens = [];

    if (tiposDePoderes.indexOf(tipoDePoder) >= 0) {
        personagens.forEach((personagem) => {
            personagem.powers.forEach((poder) =>
                poder.rarity == tipoDePoder
                    ? nomeDosPersonagens.push(personagem.full_name)
                    : ""
            );
        });
        return nomeDosPersonagens;
    } else {
        return `Tipo de Raridade Inválida`;
    }
}
console.log(buscaPersonagensComTipoDePoder("teste"));
...
function todosOsPoderes() {
    let poderes = [];
    personagens.forEach((personagem) => {
        personagem.powers.forEach((poder) => poderes.push(poder.name));
    });
    return poderes;
}
console.log(todosOsPoderes());
...
function personagensPorTipo() {
    let tipos = tiposDePersonagem();
    let filtro = [];
    tipos.forEach((tipo) => {
        let nomeDosPersonagensDoTipo = [];
        personagens.forEach((personagem) => {
            personagem.type == tipo
                ? nomeDosPersonagensDoTipo.push(personagem.full_name)
                : "";
        });
        filtro.push({ tipo: tipo, nomes: nomeDosPersonagensDoTipo });
    });

    return filtro;
}
console.log(personagensPorTipo());
...
function calculaPoder() {
    let poderCalculado = [];
    personagens.forEach((personagem) => {
        let total = 0;
        personagem.powers.forEach((poder) => {
            total += parseInt(poder.power);
        });
        poderCalculado.push({ name: personagem.full_name, totalPower: total });
    });
    return poderCalculado;
}

console.log(calculaPoder());
