...
let dirige = pessoas.map(pessoa => pessoa.idade >= 18 ? pessoa.nome + ": Pode dirigir" : pessoa.nome + ": Não pode dirigir");
console.log(dirige);
...
*      ]
*/
let apresentacao = pessoas.map(pessoa => `Olá ${[pessoa.nome, pessoa.sobrenome].join(' ')}!! você tem ${pessoa.idade}.`)
...
let idadeAlterada = pessoas.map(pessoa => ({...pessoa, idade : pessoa.idade + 1}))
console.log(idadeAlterada);
...
let capitalized = pessoas.map(pessoa => [pessoa.nome, pessoa.nome].join(' ').toUpperCase())
...
let nomecompleto = pessoas.map(pessoa => ({...pessoa, nomeCompleto: [pessoa.nome,pessoa.sobrenome].join(' ')}))
...
let quantidadeLetrasNomeSobrenome = pessoas.map(pessoa => ({ qtdeNome: pessoa.nome.length, qtdeSobrenome: pessoa.sobrenome.length }))
console.log(quantidadeLetrasNomeSobrenome);